...
void G422::setupAnimations()
{
	...
	static MGROUP_ROTATE mgRot_lgdoor_op1_CFL(mdlID_ext, lgDoors + 6, 1, V3_GEAR_DOOR_C_FL, _V(0, 0, 1), float(-PI05)); // open fore-center doors
	static MGROUP_ROTATE mgRot_lgdoor_op1_CFR(mdlID_ext, lgDoors + 7, 1, V3_GEAR_DOOR_C_FR, _V(0, 0, 1), float(PI05));

	AddAnimationComponent(an_lg, 0, 0.2, &mgRot_lgdoor_op1_CFL);
	AddAnimationComponent(an_lg, 0, 0.2, &mgRot_lgdoor_op1_CFR);

	static MGROUP_ROTATE mgRot_lgdoor_CAL(mdlID_ext, lgDoors + 4, 1, V3_GEAR_DOOR_C_AL, _V(0, 0, 1), float(-PI05)); // open aft-center doors
	static MGROUP_ROTATE mgRot_lgdoor_CAR(mdlID_ext, lgDoors + 5, 1, V3_GEAR_DOOR_C_AR, _V(0, 0, 1), float(PI05));
	...
	static MGROUP_ROTATE mgRot_lgdoor_op2_CFL(mdlID_ext, lgDoors + 6, 1, V3_GEAR_DOOR_C_FL, _V(0, 0, 1), float(PI05)); // close fore-center doors
	static MGROUP_ROTATE mgRot_lgdoor_op2_CFR(mdlID_ext, lgDoors + 7, 1, V3_GEAR_DOOR_C_FR, _V(0, 0, 1), float(-PI05));
	...
	static MGROUP_ROTATE mgRot_tswdoor_L(mdlID_ext, tailWheel + 1, 1, VHINGE_PVT_TWDOOR_L, float(-PI05));
	static MGROUP_ROTATE mgRot_tswdoor_R(mdlID_ext, tailWheel + 2, 1, VHINGE_PVT_TWDOOR_R, float(PI05));
	...
	landingGears = new MovingPart(an_lg, 0.1, MP_HI_DETENT, 1, this, G422_SYSID_LGRS);
	...
	visor = new MovingPart(an_vsr, 0.15, MP_LOW_DETENT, 0, this, G422_SYSID_VISR);
	...
	rcsDoors = new MovingPart(an_rcs, 0.3, MP_LOW_DETENT, 0, this, G422_SYSID_RCSDR);

	// inlet doors...
	UINT an_inlt = CreateAnimation(0);
	...
	inltDoors = new MovingPart(an_inlt, 0.3, MP_HI_DETENT, 1, this, G422_SYSID_INLT);
	...
	ramxDoors = new MovingPart(an_ramx, 0.25, MP_LOW_DETENT, 0, this, G422_SYSID_RAMX);
	...
	canards = new MovingPart(an_cnrd, 0.12, MP_HI_DETENT, 1, this, G422_SYSID_CNRD);
	...
	static MGROUP_ROTATE mgRot_bayDoor_L(mdlID_ext, cgoBay, 2, V3_BAY_DOOR_L, _Z, float(PI));
	static MGROUP_ROTATE mgRot_bayDoor_R(mdlID_ext, cgoBay + 2, 2, V3_BAY_DOOR_R, _Z, float(-PI));
	...
	bayDoors = new MovingPart(an_cgoBay, 0.01, MP_LOW_DETENT, 0, this, G422_SYSID_BAY);
	...
	ANIMATIONCOMPONENT_HANDLE acsPar_L = AddAnimationComponent(an_wingsDown, 0, 1, mgRot_wtL, wingPar_L);
	ANIMATIONCOMPONENT_HANDLE acsPar_R = AddAnimationComponent(an_wingsDown, 0, 1, mgRot_wtR, wingPar_R);
	...
	AddAnimationComponent(an_wingsDown, 0, 1, mgRot_wtLgtsL, wingPar_L);
	AddAnimationComponent(an_wingsDown, 0, 1, mgRot_wtLgtsR, wingPar_R);

	wingTipWvrd = new MovingPart(an_wingsDown, 0.15, MP_LOW_DETENT, 0, this);
	wingTipFthr = new MovingPart(an_wingsUp, 0.15, MP_LOW_DETENT, 0, this);
	...
	AddAnimationComponent(an_acs_cnrd, 0, 1, &mgRot_cnrdACS_L);
	AddAnimationComponent(an_acs_cnrd, 0, 1, &mgRot_cnrdACS_R);
	...
	AddAnimationComponent(an_acs_rddr, 0, 1, &mgRot_rddr);
	...
	AddAnimationComponent(an_acs_alrn, 0, 1, &mgRot_alrnACS_L);
	AddAnimationComponent(an_acs_alrn, 0, 1, &mgRot_alrnACS_R);
	...
	AddAnimationComponent(an_acs_elvr, 0, 1, mgRot_acs_evL, acsPar_L);
	AddAnimationComponent(an_acs_elvr, 0, 1, mgRot_acs_evR, acsPar_R);
	...
	gimbal = AddAnimationComponent(an_dvc_adi_A, 0, 1, &mgRot_ADI_gimbal);

	mgRot_ADI_G1 = new MGROUP_ROTATE(mdlID_dvc, ADI, 0, V3_ADI_BALL, _Y, float(PI * 2.0));
	gimbal = AddAnimationComponent(an_dvc_adi_B, 0, 1, mgRot_ADI_G1, gimbal);

	mgRot_ADI_G2 = new MGROUP_ROTATE(mdlID_dvc, ADI, 1, V3_ADI_BALL, _Xn, float(PI * 2.0));
	AddAnimationComponent(an_dvc_adi_C, 0, 1, mgRot_ADI_G2, gimbal);

	// ramx throttle
	an_dvc_ramx = CreateAnimation(0);
	...
	AddAnimationComponent(an_dvc_ramx, 0, 1, &mgRot_ramxThr);

	// ctrl stick
	an_dvc_csPitch = CreateAnimation(0);
	an_dvc_csRoll = CreateAnimation(0);
	...
	AddAnimationComponent(an_dvc_csPitch, 0, 1, &mgRot_stickPitch);
	AddAnimationComponent(an_dvc_csPitch, 0, 1, &mgRot_boot1Pitch);
	AddAnimationComponent(an_dvc_csPitch, 0, 1, &mgRot_boot2Pitch);
	...
	AddAnimationComponent(an_dvc_csRoll, 0, 1, &mgRot_stickRoll);
	AddAnimationComponent(an_dvc_csRoll, 0, 1, &mgRot_boot1Roll);
	AddAnimationComponent(an_dvc_csRoll, 0, 1, &mgRot_boot2Roll);

	// throttle unit
	an_dvc_thr = CreateAnimation(0);
	...
	AddAnimationComponent(an_dvc_thr, 0, 1, &mgRot_thrUnit);

	// wing lever
	an_dvc_wpos = CreateAnimation(0);
	...
	AddAnimationComponent(an_dvc_wpos, 0, 1, &mgRot_wPosLvr);
	...
	AddAnimationComponent(an_dvc_tiller, 0, 1, &mgRot_tiller);

	// parking brake handle
	an_dvc_prk = CreateAnimation(1);
	static UINT vcPrk[1] = { MGID_PRK_BRK_HNDL };
	static MGROUP_ROTATE mgRot_prk(mdlID_dvc, vcPrk, 1, POSDIR_PRK_BRK_HNDL, float(-PI05));
	AddAnimationComponent(an_dvc_prk, 0, 1, &mgRot_prk);

	// gear emergency release handle
	an_dvc_gear = CreateAnimation(0);
	static UINT vcGear[1] = { MGID_GEAR_EMER_HNDL };
	static MGROUP_ROTATE mgRot_gear(mdlID_dvc, vcGear, 1, POSDIR_GEAR_EMER_HNDL, float(PI05));
	AddAnimationComponent(an_dvc_gear, 0, 1, &mgRot_gear);

	// undocking handle
	an_dvc_undock = CreateAnimation(0.5);
	static UINT vcUndock[1] = { MGID_UNDOCK_HNDL };
	static MGROUP_TRANSLATE mgTran_undock(mdlID_dvc, vcUndock, 1, _V(0, -0.00818663, -0.00574273));
	AddAnimationComponent(an_dvc_undock, 0, 1, &mgTran_undock);
	SetAnimation(an_dvc_undock, 0);
	...
	{
		VCKnobDef& kbDef = VC_kbDefs[i];
		VCKnob& vcKb = VCKnobs[i];

		vcKb.middleState = kbDef.middleState;
		vcKb.pos = kbDef.init_pos;
		vcKb.mgid = kbDef.mgid;
		vcKb.anID = CreateAnimation(kbDef.initState);

		vcKb.mgRot = new MGROUP_ROTATE(mdlID_dvc, &(kbDef.mgid), 1, kbDef.refPos, kbDef.axisDir, kbDef.rotAngle);
